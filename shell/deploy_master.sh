...
for repo_entry in "${repos[@]}"; do
    ...
    # ── Git fetch & compare ──────────────────────────────────────────────────
    git fetch origin master >> "$LOG_FILE" 2>&1 || { log "ERROR: git fetch failed for ${repo_path}"; continue; }

    LOCAL=$(git rev-parse HEAD)
    REMOTE=$(git rev-parse origin/master 2>/dev/null)

    if [ -z "$REMOTE" ]; then
        log "ERROR: Cannot resolve origin/master for ${repo_path}"
        continue
    fi
    ...
    git pull origin master >> "$LOG_FILE" 2>&1 || { log "ERROR: git pull failed for ${repo_path}"; continue; }
